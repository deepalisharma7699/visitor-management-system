    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-green-600">{{ $syncedVisitors }}</p>
            </div>
        ...
        @if ($totalVisitors > 0)
            <div class="mt-2">
                <div class="flex items-center justify-between text-sm">
                    ...
                    <span class="font-medium">{{ round(($syncedVisitors / $totalVisitors) * 100) }}%</span>
                </div>
                <div class="mt-1 w-full bg-gray-200 rounded-full h-2">
                    <div class="bg-green-600 rounded-full h-2" 
                         style="width: {{ ($syncedVisitors / $totalVisitors) * 100 }}%"></div>
                </div>
            </div>
        @endif
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-orange-600">{{ $unsyncedVisitors }}</p>
            </div>
        ...
        @if ($unsyncedVisitors > 0)
            <form action="{{ route('admin.sync.manual') }}" method="POST" class="mt-3">
                @csrf
                <button type="submit" 
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($recentUnsynced as $visitor)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                #{{ str_pad($visitor->id, 5, '0', STR_PAD_LEFT) }}
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs font-semibold rounded-full
                                    {{ $visitor->visitor_type === 'guest' ? 'bg-blue-100 text-blue-800' : '' }}
                                    {{ $visitor->visitor_type === 'broker' ? 'bg-green-100 text-green-800' : '' }}
                                    {{ $visitor->visitor_type === 'customer' ? 'bg-purple-100 text-purple-800' : '' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $visitor->verified_at->format('M d, Y H:i') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
